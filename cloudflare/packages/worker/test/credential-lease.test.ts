...
describe("tenant credential leases", () => {
  ...
  test("applies account-scoped forbidden cooldowns across models", async () => {
    const worker = await startWorker()
    const tenant = await createTenant(worker.baseURL, "Forbidden account")
    const first = await fetch(`${worker.baseURL}/tenant/accounts`, {
      method: "POST",
      headers: tenantHeaders(tenant.key),
      body: JSON.stringify({
        ...
        apiKey: "sk-ant-organization-disabled",
      }),
    })
    const firstBody = await first.json() as { id: string }
    expect(first.status).toBe(200)
    const second = await fetch(`${worker.baseURL}/tenant/accounts`, {
      ...
      body: JSON.stringify({
        ...
        apiKey: "sk-ant-account-fallback",
      }),
    })
    expect(second.status).toBe(200)
    ...
    const opus = await fetch(`${worker.baseURL}/tenant/leases`, {
      ...
      body: JSON.stringify({
        provider: "claude",
        agentType: "claude",
        sessionId: "forbidden-account-opus",
        preferAccountId: firstBody.id,
        model: "claude-opus-4-8",
      }),
    })
    const opusBody = await opus.json() as { leaseId: string; token: string }
    expect(opus.status).toBe(200)
    expect(opusBody.token).toBe("sk-ant-organization-disabled")
    ...
    const sonnet = await fetch(`${worker.baseURL}/tenant/leases`, {
      ...
      body: JSON.stringify({
        provider: "claude",
        agentType: "claude",
        sessionId: "forbidden-account-sonnet",
        preferAccountId: firstBody.id,
        model: "claude-sonnet-4-6",
      }),
    })
    const sonnetBody = await sonnet.json() as { token: string }
    expect(sonnet.status).toBe(200)
    expect(sonnetBody.token).toBe("sk-ant-account-fallback")
  }, 60_000)
