...
// session's own account for up to ~30s (~10s when an egress or Azure
// fallback is configured), keeping its prompt cache.
...
// X-Subrouter-Capacity-Retry headers are honored only with the failover on or
