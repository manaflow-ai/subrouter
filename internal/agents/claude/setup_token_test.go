...
func TestRefreshPathsKeepSetupTokenUsableUntilExpiry(t *testing.T) {
	...
	// This release predates Store.CredentialRefreshState, so the expiry
	// semantics are asserted directly on the shared helper both refresh paths
	// call.
	...
	if err := longLivedCredentialError("work", &credential, afterExpiry); err == nil ||
		...
		t.Fatalf("expired long-lived credential error = %v", err)
	}
